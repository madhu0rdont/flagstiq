...
- Action space: every eligible club × 13 aim bearings (5° increments across ±30°) per zone
- Transition sampling: 200 Gaussian shots per action with tree collision + hazard checks
- Value iteration with 3 scoring modes: Scoring (minimize expected strokes), Safe (risk-adjusted), Aggressive (birdie hunt)
- Policy extraction + 2,000 Monte Carlo trials for accurate score distributions
- All computation server-side (~4-5s/hole, ~80s for 18 holes)
- New `POST /api/strategy/hole` endpoint for per-hole strategy fetch
- New `POST /api/game-plans/:courseId/:teeBox/:mode/generate` endpoint for server-side plan generation
- Client hooks (`useHoleStrategy`, `useGamePlanCache`) migrated from local computation to server API calls
- Client bundle reduced ~11KB (strategy computation removed from browser)
- Template-based fallback retained when DP returns no results (e.g., missing geometry)
- Migration flag `strategy_sync_v3` forces plan regeneration on deploy
